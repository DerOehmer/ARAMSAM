...
class App:
    ...
    def set_sam(self):
        if self.experiment_mode is None:
            background_embedding = self.configs.sam_background_embedding
    ...
    def select_next_img(self):
        self.ui.enable_push_buttons()
        if not self.img_fnames:
            print("No image left in the queue")
            self.ui.disable_push_buttons()
            if self.experiment_mode in ["structured", "polygon"]:
                ...
                reply = self.ui.create_message_box(
                    False,
                    "No more images left in current directory. Check output directory whether annotations were already done. Load a new folder/image to proceed",
                    wait_for_user=True,
                )
                ...
                if reply and self.annotator.annotation is not None:
                    self.ui.save()
        ...
        if current_img_done and next_img_done:
            ...
            if self.configs.save_data.bbox_style == "default":
        ...
        if self.sam_gen == 2:
            ...
            if self.experiment_mode == "tutorial":
                ...
                or not self.configs.sam_background_embedding
            ):
    ...
    def embed_img_pair(self):
        ...
        img_embed_worker = Sam2ImgPairEmbeddingWorker(
            self.annotator.sam, img_pair, self.configs.do_amg, self.mutex
        )
    ...
    def receive_amg_results(self, result: tuple):
        ...
        now = time.time()
        if len(self.annotator.annotation.masks) > 0:
